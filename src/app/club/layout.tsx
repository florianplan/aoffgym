...
import {
  Gauge, Users, Handshake, BarChart3, Info, FolderOpen, ReceiptText,
  LogOut, Menu, X, Newspaper, CalendarDays, Mail,
} from 'lucide-react';
import type { LucideIcon } from 'lucide-react';

const navItems: { href: string; label: string; Icon: LucideIcon }[] = [
  { href: '/club',               label: 'Tableau de bord', Icon: Gauge },
  { href: '/club/licencies',     label: 'Licenciés',       Icon: Users },
  { href: '/club/reaffiliation', label: 'Ré-affiliation',  Icon: Handshake },
  { href: '/club/statistiques',  label: 'Statistiques',    Icon: BarChart3 },
  { href: '/club/infos-club',    label: 'Infos club',      Icon: Info },
  { href: '/club/documents',     label: 'Documents',       Icon: FolderOpen },
  { href: '/club/decomptes',     label: 'Décomptes',       Icon: ReceiptText },
];
...
function SidebarContent({ pathname, onNavigate, showLogo = true }: { pathname: string; onNavigate?: () => void; showLogo?: boolean }) {
  return (
    <>
      ...
      {/* Sélecteur d'espace */}
      <SpaceSwitcher currentSpaceId="club" />

      {/* Navigation */}
      <nav className="flex-1 px-3 py-3 space-y-0.5 overflow-y-auto" aria-label="Navigation espace club">
        {navItems.map((item) => {
          const isActive = item.href === '/club'
            ? pathname === item.href
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="lg:hidden sticky top-0 z-30 flex items-center gap-3 px-4 py-3 bg-white/90 backdrop-blur-lg border-b border-slate-100">
          ...
          <span className="flex-1 text-sm font-semibold text-slate-500 truncate">
            {navItems.find(i =>
              i.href === '/club' ? pathname === i.href : pathname === i.href || pathname.startsWith(i.href + '/')
            )?.label ?? 'Espace Club'}
          </span>
          <button
